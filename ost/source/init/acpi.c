...
// acpi电源管理
// NOTE： i copy the code below from "http://forum.osdev.org/viewtopic.php?t=16990"
...
// all of these macros are wrote to make the code i copyed to work
#define wrstr(x) ost_puts(x)
#define outb(x, y) outbyte(x, y)
#define outw(x, y) outword(x, y)
#define inw(x) inword(x)
#define NULL 0
#define byte char
#define dword int
#define word short
...
#define sleep(x) do{ \
  int a = get_ticks(); \
  while(get_ticks() - a < x * 16); \
}while(0)

static int memcmp(void* a, void* b, int x){
  for(int i = 0; i < x; i++){
    if(*(char*)a != *(char*)b){
      return 1;
    }
