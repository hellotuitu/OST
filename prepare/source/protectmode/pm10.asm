...
LABEL_MEM_CHK_OK:
  ...
  mov edx, label_seg_interupt
  mov esi, label_desc_interupt
  call load_desc
	; 为加载 GDTR 作准备
	xor	eax, eax
	mov	ax, ds
...
label_seg_data:
  ...
  message_user_int db "hello world, i am 80h int!"
  message_user_int_len equ $ - message_user_int
  memory_num dd 0
...
label_seg_32_code:
  ...
  ; int 80h
  call selector_interupt:init_8259a
  ; int 20h
  int 80h
  sti
  jmp $
...
label_seg_interupt:
  init_8259a equ $ - label_seg_interupt
  mov	al, 011h
  ...
  clock_handler equ $ - label_seg_interupt
    mov ebx, clock_counter - label_seg_data
    mov edx, 72
    call selector_print:print_dword
    inc dword [ds:(clock_counter - label_seg_data)]
    mov	al, 20h
    out	20h, al				; 发送 EOI
    iretd ; iret will recover flags register
          ; in 32bit mode, iretd equal to iret
          ; iretd will pop a 32bit eip
  spurious_handler equ $ - label_seg_interupt
		call io_delay
    iretd

  user_handler equ $ - label_seg_interupt
    mov ebx, message_user_int - label_seg_data
    mov ecx, message_user_int_len
    mov edx, 80*10 + 40
    call selector_print:0
    ; mov	al, 20h
    ; out	20h, al				; 发送 EOI
...
seg_interupt_len equ $ - label_seg_interupt
...
label_desc_interupt: Descriptor 0, seg_interupt_len - 1, DA_C | DA_32
...
selector_interupt equ label_desc_interupt - label_gdt
...
%rep 32
			Gate	selector_interupt, spurious_handler,      0, DA_386IGate
%endrep
.020h:			Gate  selector_interupt, clock_handler,      0, DA_386IGate
%rep 95
			Gate	selector_interupt, spurious_handler,      0, DA_386IGate
%endrep
.080h:			Gate	selector_interupt, user_handler,      0, DA_386IGate
